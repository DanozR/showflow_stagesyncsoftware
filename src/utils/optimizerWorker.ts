// Web Worker for optimizing show order
...
self.onmessage = (event) => {
  const { classes, minGap, type } = event.data;
  
  if (type === 'optimize') {
    const result = optimizeShowOrder(classes, minGap);
...
export const optimizeShowOrder = (
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    // Score this attempt (higher is better)
